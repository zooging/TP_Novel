...
screen reed_info():
    ...
    vbox:
        ...
        text ""
        text " "
        text "Тревожное расстройство это..."
        text " "
        text "{size=-13}Генерализованное тревожное расстройство (ГТР) — это психическое расстройство, которое проявляется в чрезмерном беспокойстве в повседневной жизни и распространяется на разные сферы: отношения, семью, финансы, работу, учебу, здоровье, политические и мировые дела. Людям с ГТР свойственно ощущение, что они не владеют контролем над ситуацией, что что-то обязательно должно пойти не так и даже может закончиться катастрофой. Они избегают событий, которые могут иметь негативные последствия, а также тратят значительное время на подготовку к сложным мероприятиям."
...
screen reed_info2():
    ...
    vbox:
        ...
        text " "
        text "{size=-13}На сегодняшний день четкого понимания о том, как возникают тревожные расстройства, нет. Эта патология может развиваться под воздействием различных психических и соматических факторов.Тревога может возникнуть в ответ на внешние раздражители, при различных соматических заболеваниях, травмах головного мозга и эндокринных нарушениях. Тревожные состояния могут быть вызваны приемом некоторых лекарственных препаратов, наркотических и психотропных средств.Существует несколько теорий, объясняющих причины возникновения тревожных патологий"
...
screen reed():
    ...
    imagemap:
        ground "gui/menu_reed.png"
        idle "gui/menu_reed_normal.png"
        hover "gui/menu_reed_hover.png"
        #Высота +107

        hotspot(45,58,408,67) action  NullAction()
        hotspot(65,146,334,54) action [Hide("reed_info1"), Hide("reed_info2"), Hide("reed_info3"),Hide("reed_info4"), Show("reed_info")]
        hotspot(65,223,334,54) action [Hide("reed_info"), Hide("reed_info2"), Hide("reed_info3"),Hide("reed_info4"), Show("reed_info1")]
        hotspot(65,300,334,54) action [Hide("reed_info"), Hide("reed_info1"), Hide("reed_info3"),Hide("reed_info4"), Show("reed_info2")]
        hotspot(65,375,334,54) action [Hide("reed_info"), Hide("reed_info1"), Hide("reed_info2"),Hide("reed_info4"), Show("reed_info3")]
        hotspot(65,450,334,54) action [Hide("reed_info"), Hide("reed_info1"), Hide("reed_info3"),Hide("reed_info2"), Show("reed_info4")]
        hotspot(38,676,257,38) action [Hide("reed_info"), Hide("reed_info1"), Hide("reed_info3"),Hide("reed_info2"),Hide("reed_info4"), Return()]
...
screen character():
    # Картинка
    add "gui/pchel.png" xpos 0.33 yalign 0.14 xsize 580 ysize 650
    ...
    vbox:
        ...
        text "Подросток-Ян"
        text " "
        text "{size=-13}Подросток, который рассказывает гг о корабле и куда все плывут. Боится высоты и крика, потому что в детстве в приюте с ним плохо обращались. После приюта решил стать отважным мореплавателем, думал, что так у него получится сбежать от прошлого. В процессе путешествия понял, что это не так, надо не пытаться забыть плохие моменты,а принять их и научиться с ними жить. Использует дыхательную технику и настраивает себя на том, что все получится в будущем, чтобы успокоиться."

screen character2():
    # Картинка

    add "gui/pchel2.png" xpos 0.33 yalign 0.14 xsize 580 ysize 650
    ...
    vbox:
        ...
        text ""
        text " "
        text "Писатель-Якоб Хенц"
        text " "
        text "{size=-13}Писатель, испытывает мышечное напряжение и невроз при стрессовых ситуациях. Пишет книгу уже 10 лет и никак не может закончить. Переписывал концовку 8 раз. Несколько раз рукопись портилась (ее заливал водой неумелый официант, и чернила текли, как-то ее сжег заядлый спорщик в порыве, и выкидывала невнимательная горничная). Уже смирился, что не все в жизни может контролировать. Всегда носит с собой блокнот и ручку, чтобы во время стресса “отчертить тревожность” и выкинуть ее, иногда рисует."

screen character3():
    # Картинка

    add "gui/pchel3.png" xpos 0.33 yalign 0.14 xsize 580 ysize 650
    ...
    vbox:
        ...
        text ""
        text " "
        text "Оперный певец-Андреа Каррерас"
        text " "
